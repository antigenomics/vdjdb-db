...
pm_data <- content(
  POST("https://eutils.ncbi.nlm.nih.gov/",
...
p1=ggplot(dt.vdjdb.s2, aes(x = as.integer(pub_date2), y = tcr_count, color = chains)) +
  ...
  annotate("segment", x = 2025, xend = 2025, y = 0, yend = 75000, linetype="solid", color = "grey25", size = 0.3) +
  annotate("text", x = 2025, y = 77000, label = "phage\ndisplays", hjust = 1, vjust = 1) +
  geom_line() +
  geom_point() +
  ylab("") +
  scale_x_continuous("", breaks = seq(1996, 2026, by =2), limits = c(1996, 2026)) +
